...
素数在数学之中类似空气存在于地球，无处不在而且非常重要。本章首先简要地陈述素数的若干性质，为以后章节铺垫基础，然后重点讲解一种判断整数是否素数的算法。
\section{素数}
\label{sec:prime}
...
通过简单的 SageMath 命令，我们可以稍微建立起一些关于素数分布的基本认识。
\begin{lstlisting}
sage: prime_pi(1000)
168
sage: prime_pi(10000)
...
以下是用 SageMath 命令画出的一万以内整数中的素数分布图，大家可以尝试使用其他参数，取$1000$或者$100,000$等
得到的效果图基本相同。
%Add 20200723
\begin{figure}
\begin{center}
\includegraphics[scale=.5]{figures/chapt5-1-prime-counting-function.pdf} 	
...
\section{素性测试}
\label{sec:primeTest}
本节讨论素性测试问题，即如何判断一个大的奇整数$n$是否素数。先看以下 SageMath 实例。
\begin{example}
\label{exm:prime}
\begin{lstlisting}
sage: is_prime(131)
...
对于某些特定的算法，往往并不关心某个特定的输入值，而关注满足某一个\emph{规模}的输入值，此时衡量算法的复杂性的输入就应该是输入值的数据长度。
...
进一步要判断是否$a^{(n-1)/2} \equiv \pm 1 \pmod{n}$。如果这个同余式不成立，则可判断$n$为合数。以下实例说明如何通过这种方法发现 Carmichael 数不是素数。%20250625改写。
...
以上证明也就给出了米勒测试算法的基本过程，以下代码给出了算法的具体描述。
...
米勒与拉宾的想法结合起来，这就形成了米勒-拉宾素性测试算法\index{米勒-拉宾算法}\index{Miller-Rabin primality test}：通过$k$次不同的米勒测试，可以把合数误判为素数的概率尽可能地降低。
代码如下：
\begin{center}
...
设计出一种后来命名为 AKS 的确定性算法~\cite{AKS04primes}(正式发表于2004年)，这在当时的学术圈是非常轰动的大事件\index{AKS 算法}。AKS 是一种通用的多项式时间确定性算法，
