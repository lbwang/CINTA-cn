...
这一章将讲解这样一段优美的“故事”：通过计算发现模式，总结规律，并证明定理。或者，这只是描述数学家们枯燥无味的日常工作？不，相信这是一个有趣的故事。

首先，我们试图从模乘法和模指数运算中发现一些规律。先看这样一种情况，给定素数$p$和一个正整数$a$，
然后观察$a \bmod p$, $a^2 \bmod p$, $a^3 \bmod p$, $\ldots$。你能找出其中的一些规律吗？
也许你可以尝试使用以下代码。
...
你能找到其中的一些规律吗？可通过设$p = 11$或者其他数值，运行代码来验证你自己的猜想。在$p = 11$的情形下，我们得到表~\ref{pow_a_mod_11}。
...
观察表~\ref{ai_mod_7}，一种非常容易发现的规律就是：
...
\begin{thinking}
	提醒读者思考以上证明中\emph{置换}与\emph{消去律}之间的关系。
\end{thinking}
...
\begin{figure}[htbp]
	\centering
	\input{figures/fermat-little-theorem-cycle.tikz}
	\caption{费尔马小定理的直观}
	\label{fig:flt-cycle}
\end{figure}
...
\section{欧拉定理}
\label{sec:euler_thm}
在上一节的分析中，模数是一个素数，借助消去律，得到了费尔马小定理。如果模数是合数，则消去律就可能失效了。比如，设 $n = 6$，
借助之前的代码，可以得到以下数据。
\begin{table}[htbp]
	\centering
		\begin{tabular}{ | c | c | c | c | c | c | }
...
表~\ref{ai_mod_9}中标记为绿色的两行展示了这个事实。请读者自己认真检验这一观察结果，或者做更多的计算并观察。

然而，从乐观的角度看，置换属性还是有可能得到保持的。将\textbf{不}互素于$n$的“坏家伙”从表格中剔除，将得到表~\ref{ai_mod_9_2}。
...
\begin{figure}[htbp]
	\centering
	\input{figures/euler-theorem-cycle.tikz}
	\caption{欧拉定理的直观}
...
本节的主要任务是计算 $\varphi(mn)$，其中，$m$和$n$是互素的正整数。 分析分为两大步骤。
...
本章讲解费尔马小定理、欧拉定理的思路源自于 Silverman 的 FINT~\cite{silverman2014friendly}，本书作者从中学到了许多知识与方法，也收获了许多的乐趣。想进一步了解欧拉 Phi 函数，或者积性函数相关知识点的读者，建议从 Bur11~\cite{Bur11} 入手。

\begin{problemset}

  \item 设$p = 23$和$a = 3$，使用费尔马小定理计算$a^{2019} \bmod p$？ %ans: 16, 因为 3^7 = 2

  \item 找规律：请尽可能多地归纳总结表~\ref{pow_a_mod_5}到表~\ref{pow_a_mod_11}中数字的规律。
