...
\frame{\frametitle{Review: Finding the patterns.}
\begin{block}{Review.}
...
Given $p$ be an odd prime and $y$ an integer relatively prime to $p$, is $y$ a perfect square modulo $p$?  Equivalently, we need to decide whether there exists $x\in \Z$ such that:
\[ x^2 \equiv  y \pmod{p}. \]
\end{exampleblock}
}

\frame{\frametitle{A Na\"{\i}ve Solution.} 
...
The second column of each table is symmetric under reflection (反射对称), as described by the following formula.
 
 \[(p - a)^2 \equiv a^2 \equiv b \pmod{p}.\]
\end{block}
\pause
 ...
 This formula means the congruence $x^2 \equiv b \pmod{p}$ has two incongruent solutions, $a$ and $p - a$. 
}
...
Assume that $x^2 \equiv b \pmod{p}$ has a solution. 
...
\begin{example}
  From these tables we see that, the $\mathbf{QR}$ modulo $7$ are $\{1, 2, 4\}$; the $\mathbf{QR}$ modulo $13$ are $ \{1, 3, 4, 9, 10, 12\}$; the $\mathbf{QNR}$ modulo $7$ are $\{3, 5, 6\}$; the $\mathbf{QNR}$ modulo $13$ are $\{2, 5, 6, 7, 8, 11\}$.
  \end{example}
}
...
\frame{\frametitle{QR and QNR.}
\begin{proof}
 We map the integers from $\Z_p^*$ to $\mathbf{QR}$s modulo $p$ by squaring. Since Proposition~\ref{prop:num_qr} tells us that, each $\mathbf{QR}$ has exactly two preimages under the squaring map. We have $p-1$ squares to consider, then there are exactly $(p-1)/2$ $\mathbf{QR}$s modulo $p$. The remaining $(p-1)/2$ integers are $\mathbf{QNR}$s modulo $p$.
 \end{proof}
}
...
\begin{proposition}
  \label{prop:QR_gr}
  Let $\mathbb{QR}_p$ denote the set of every $\mathbf{QR}$ modulo $p$, $\mathbb{QR}_p$ is a group under multiplication. 
  \end{proposition}
  
  \begin{proof}
  It is easy to prove by checking all the group axioms.
  \end{proof}
}

\frame{\frametitle{Quadratic Residues Group.}
\begin{block}{Remark.}
The map $\phi:\mathbb{Z}_p^* \to \mathbb{QR}_p$ is a group homomorphism. From earlier observation we know $\ker \phi = \{1, p-1\}$. By the First Isomorphism Theorem from Chapter 9, this gives another proof of the previous theorem.
\end{block}
...
\frame{\frametitle{Quadratic Residues vs Quadratic Non-Residues.}
 ...
 (1.) The product of two $\mathbf{QR}$s modulo $p$ is a $\mathbf{QR}$, denoted as
 \[
 \mathbf{QR} \times \mathbf{QR} = \mathbf{QR},
 \]
 
 (2.) The product of a $\mathbf{QR}$ modulo $p$ and a $\mathbf{QNR}$ modulo $p$ is a $\mathbf{QNR}$ modulo $p$, denoted as
 \[
 \mathbf{QR} \times \mathbf{QNR} = \mathbf{QNR},
 \]
 
 (3.) The product of two $\mathbf{QNR}$s modulo $p$ is a $\mathbf{QR}$, denoted as
 \[
...
\frame{\frametitle{Quadratic Residues vs Quadratic Non-Residues.}
 \begin{proof}
  ...
  \[ c^2(a_1^{-1})^2 \equiv b \pmod{p}.\] This means that $b$ is a $\mathbf{QR}$ contradicting the assumption that $b$ is a $\mathbf{QNR}$.
  \end{proof}
}
...
\frame{\frametitle{以上证明的代数视角.}
    ...
    \item 为什么$\mathbf{QR} \times \mathbf{QNR} = \mathbf{QNR}$？答：因为$e*a = a$。
    
    \item 为什么$\mathbf{QNR} \times \mathbf{QNR} = \mathbf{QR}$？答：$a*a$只能等于$e$。
    \end{itemize} 
    \end{block}
\pause
    \begin{block}{Remark.}
      一种更常见、更直观的记法是把群$\{e, a\}$记为乘法群$\{ 1, -1\}$。即把$\mathbf{QR}$记为$1$，把$\mathbf{QNR}$记为$-1$。请读者自行验证。
...
\section{Legendre Symbol and Euler's Criterion.}
\label{sec:ls_ec}
...
\begin{proposition}
   ...
   \pause
    ...
    \begin{proof}
    This follows directly from Proposition~\ref{prop:qr_mul}. 
    \end{proof}
}
...
\frame{\frametitle{Euler's Criterion.}
  \begin{proof}
  We have two cases to consider. For the first case, we assume $\left(\frac{a}{p} \right) = 1$, which means that there exists $x \in \Z$ such that $x^2 \equiv a \pmod{p}$. By Fermat's little theorem, we have that
...
\frame{\frametitle{Euler's Criterion: Finding the patterns.}
\begin{block}{Euler's Criterion.}
Let $p = 7$, and for very $1\leq a < p$, compute $a^i\; \text{mod}\; p$, where $1\leq i < p$. We have:
\begin{center}
		\begin{tabular}{cccccc}
			\toprule
...
\end{block}
 ...
 \begin{proof}
 假设模$4$余$1$的素数有限，枚举之$S = \{1, p_1, p_2, \ldots, p_n\}$。
 令
 \begin{equation}
 \label{eq:mod4_1}
 ...
 \[(2p_1p_2 \ldots p_n)^2 \equiv -1 \pmod{p}\]
 即$\left(\frac{-1}{p}\right) = 1$。
 根据勒让德符号的属性，可知$p$是形如$4k + 1$的素数，所以$p \in S$。这说明
 $p \mid (N - (2p_1p_2 \ldots p_n)^2 )$，即$p \mid 1$，矛盾！
 \end{proof}
...
\frame{\frametitle{Exercises.} 
	...
	\item 设$n= 7\cdot11$，请手动计算求出同余方程$x^2 \equiv a \pmod{n}$在$a = 5$和$a = 4$时的所有解。%ans，a = 5时无解: 
