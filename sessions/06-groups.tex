...
在这一章的开始，首先需要思考的就是：在前两章的推导过程中，还使用了什么数学属性，但没有明确地定义或指出？比如，我们必须使用\emph{结合律}： 
...
群是集合 $\mathbb{G}$ 以及在其元素上所定义的操作符$\cdot$，并满足以下公理：
	\begin{itemize}
		...
		\item $\forall a \in \mathbb{G}$，存在\emph{逆元}（\emph{inverse}）$a^{-1}\in \mathbb{G}$，使得$a\cdot a^{-1} = e = a^{-1} \cdot a$。
	\end{itemize}
如果对任意$a, b\in \mathbb{G}$，满足$a \cdot b = b \cdot a$，则群$\mathbb{G}$被称为阿贝尔群\index{Abelian group}或交换群\index{commutative group} 。
\end{definition}
...
\begin{example}
 所有的偶数在加法下成群，记为 $2\Z$；但是奇数集合并入 $0$ 后仍不是加法群，因为奇数加奇数可能是偶数，不满足封闭性。
...
\begin{figure}
	\begin{center}
		\includegraphics[scale=1.5]{figures/chapt6-1-eighth-roots-of-unity.pdf} 	
		\caption{$8$次单位根群}
...
%20250316 添加
\begin{thinking}
	读者能发现$\Z_n$ 与$\mathbb{U}_n$ 的操作行为很类似吗？这会意味什么呢？
	另外，$\mathbb{U}_n$ 的操作行为与$\Z_p^*$并不很类似，是吗？又或者图~ref{fig:flt-cycle}与图~\ref{fig:nth_root_unity}显得如此相似会提醒你一些什么？
\end{thinking}
...
\index{圆圈群}\index{cycle group}
 ...
 %20200527
必须强调，就严谨性而言，群的定义必须包括两部分，数据集与操作符。然而为了可读性与使用的方便，符号误用无处不在。比如，对某些约定俗成的群，加法与乘法含义明确时，我们就不再注明操作符，比如$\Z$、$\Z_p$和$\Z_n$是加法群，$\Z_p^*$和$\Z_n^*$是乘法群。再比如，群元素的操作$a \cdot b$往往在不引起误解的时候也写成$ab$。为了方便，值得牺牲部分严谨性。
...
\end{proposition}
	\begin{proof}
		 留作课后练习。\qed
	\end{proof}
命题~\ref{pro:cancel_gr}告诉我们，对于群消去律必然存在。对于非交换群，还分别有左消去律和右消去律。那么，之前依赖消去律所得的若干属性就立即可以推广到任意的群上去。
...
%20190926
%20200525
要分析一个大的数据集，我们往往会先把数据划分为若干小的集合进行分析。同样地，要考察一个群的属性，也会先把群分成更小的群，然后考察这些小群的性质，从而得到群本身的性质。这种想法引出了\emph{子群}（\emph{subgroup}）\index{subgroup}的定义。通过子群来研究群的结构将是以下章节的重要主题。
...
Lang 的 Algebra~\cite{Lang12}非常著名，属于研究生水平的数学教材，从中本书作者收获了很多其他课本没有的知识点，然而并不推荐初学者直接阅读。
...
它们分别是 Diffie-Hellman 算法~\cite{DH76}和 RSA 算法~\cite{RSA78}的数学基础，这两种算法的提出者分别获得了2015年和2002年的图灵奖。
\begin{problemset}

\item 给出命题~\ref{pro:cong_eq}的完整证明。

\item 在定义~\ref{def:grp}中，单位元$e$和逆元都要求具有\emph{交换性}。对单位元要求，对任意群元$a$，有$ea = ae = a$；对逆元
要求，对任意群元$a$，存在$a^{-1}\in \mathbb{G}$，使得$a\cdot a^{-1} = e = a^{-1} \cdot a$。
