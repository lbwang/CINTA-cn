...
容易证明任意整除 $a - b$ 和 $b$ 的整数必然整除$a$和$b$，因此，$\gcd(a-b, b) \leq \gcd(a, b)$。 这种证明方法频繁用于证明两个集合的相等，本质上以上证明也是证明两个集合（$a$与$b$公因子的集合与$a-b$与$b$公因子的集合）的相等，值得初学者重视。

\noindent\textbf{效率.} 要理解 $\gcd$ 算法是一种高效算法，我们必须意识到$a \bmod b$这步操作对输入数据作出了重大的消减。容易验证：
\begin{lemma}{}{}
	如果$a \geq b$，则$(a \bmod b ) < a/2$。 
\end{lemma}
如果该引理成立，那么递归算法连续调用两次后，$a$ 与 $b$ 的值都至少消减一半，意味着算法在 $\mathcal{O}(\log a)$步之后会终止。验证引理的正确性很容易，
因为如果$b < a/2$，由除法算法即得。如果$b > a/2$， 则$(a \bmod b) = a - b$，证完。
进一步，容易看出递归算法执行第一次调用后，两个参数的值都不会超过原有的$b$，所以算法的的复杂性是 $\mathcal{O}(1 + \log b)$。当$b\geq 2$时，等价地简记为$\mathcal{O}(\log b)$，这是一种更好的上界。\\
...
显然，集合$S$非空，根据良序原则，取其中最小值$d = ar + bs$。然后证明两个小结论：$d$是$a$和$b$的公因子；
...
有一个不难证明的结论，陈述如下。下一章将会使用。
\begin{property}{(最大公因子的性质--2)}
\label{prop:gcd2}
\begin{center}
	\begin{tcolorbox}[colback=green!5!white, colframe=red!75!black,width = 11cm]
$a$ 和 $b$ 的任意整数线性组合都是 $\gcd(a,b)$ 的整数倍。而且，
任意$\gcd(a,b)$ 的整数倍都是$a$ 和 $b$ 的整数线性组合。
	\end{tcolorbox}
	\end{center}
\end{property}
