...
\begin{figure}
\begin{center}
\includegraphics[scale=.5]{figures/ec-R1.jpg} 	
...
\begin{figure}
\begin{center}
\includegraphics[scale=.5]{figures/ec-R2.jpg} 	
...
\begin{lstlisting}
sage: E3 = EllipticCurve(RR,[RR.random_element(), RR.random_element()]) #RR代表有理数域
...
\begin{figure}
\begin{center}
\includegraphics[scale=.5]{figures/ec137.jpg} 	
...
所谓无穷远点$\mathcal{O}$就是加法的单位元。这里的内涵比较丰富，稍后再进一步讨论。
当前迫切的任务是要把点的加法定义出来。
...
\begin{figure}
\begin{center}
\includegraphics[scale=.4]{figures/ec-Add1.JPG} 	
\caption{椭圆曲线加法-1}
...
\begin{figure}
\begin{center}
\includegraphics[scale=.5]{figures/ec-Add2.JPG} 	
\caption{椭圆曲线加法-2}
...
\begin{figure}
\begin{center}
\includegraphics[scale=.4]{figures/ec-InfPot.JPG} 	
...
\begin{figure}
\begin{center}
\includegraphics[scale=.5]{figures/ec-singular1.jpg} 	
\caption{带奇异点的曲线-1}
...
\begin{figure}
\begin{center}
\includegraphics[scale=.5]{figures/ec-singular2.jpg} 	
\caption{带奇异点的曲线-2}
...
因为，把$k$看为二进制数$k = \sum_{i=0}^n k_i 2^i$，$n$是某个整数，$k_i \in \{0,  1\}$代表$k$二进制表达的一个比特。
则：
\begin{align*}
...
%%2023年6月26日
 ...
 椭圆曲线是数论及其相关领域（比如密码学等）的重要研究课题，其中包含了丰富的数学知识。
 在理论研究中，椭圆曲线在费尔马大定理的证明中起到重要作用，这是一次震动全世界的理论突破。
 在实践中，椭圆曲线被广泛地应用在不同的公钥密码体制中，比如：ECDSA 签名标准~\cite{DSS13}、
