...
对折，平方数部分竟然是重合的。用公式表达如下：
 ...
 注意，$p-a \equiv a \pmod{p}$当且仅当$a = 0$，也就是说，如果$a \ne 0$的时候，$a$与$p-a$模$p$不同余。
 把这个结论写成命题如下。
...
还可以进一步把以上子群$\{e, a\}$看为乘法子群$\{1, -1\}$。这样，$\mathbf{QR} \times \mathbf{QNR} = \mathbf{QNR}$就是因为$1 \cdot -1 = 1$，而$\mathbf{QNR} \times \mathbf{QNR} = \mathbf{QR}$是因为$-1 \cdot -1 = 1$。这是一种更直观的理解。
...
在看以下证明之前，建议读者先自行完成以下思考。
%20260629
\begin{thinking}
  如果$a$是一个平方数，即存在$b$使得$b^2 \equiv  a \pmod{p}$，那么，$a^{(p-1)/2} \bmod p$必然等于什么？进一步思考，如果$a$不是一个平方数，那么$a^{(p-1)/2} \bmod p$等于什么？
\end{thinking}
...
如果$\Ker\; \psi = \Ker\; \phi$，则欧拉准则得证。两个方向，$\Ker\; \psi \subset \Ker\; \phi$
和$\Ker\; \phi \subset \Ker\; \psi$。具体细节请读者自行完成，留作课后练习。
...
最后，根据高斯引理的结论可得：
 ...
 \qed
  ...
  \begin{figure}
  \begin{center}
  \includegraphics[scale=1]{figures/chapt11-1-quadratic-reciprocity-proof.pdf} 	
  \caption{二次互反律的证明.}
