...
\begin{definition}[Ring(环)]
环$R$是一个非空集合，在$R$上有两种封闭的二元操作：加法（记为$+: R \times R \to R$）
和乘法（记为$*: R \times R \to R$），并且满足以下条件：
\begin{enumerate}
...
\frame{\frametitle{Ring(环).}
可具体地表示为以下公式。对任意$a,b,c\in R$，环$R$满足以下公理：
\begin{align*}
a + (b + c ) &= (a + b) + c && \text{（加法结合律）}\\
a + b &= b + a && \text{（加法交换律）}\\
a + 0 &= 0 + a && \text{（加法单位元）}\\
a + (- a) &= (- a) + a = 0 && \text{（加法逆元）}\\
a *(b * c) &= (a *b) * c && \text{（乘法结合律）}\\
%a * 1 &= 1*a = a && \text{（乘法单位元）}\\
(a + b) * c &= (a * c) + (b * c) && \text{（右分配律）}\\
a * (b + c) &= (a * b) + (a *c) && \text{（左分配律）}
\end{align*}
}

\frame{\frametitle{Ring(环).}
\begin{definition}[交换环、带单位元的环]
\begin{itemize}
...
\pause
\begin{alertblock}{特别提醒}
    本课程通常只考虑带单位元的环。在不特别说明的情况下，环$R$默认是带单位元的环。
\end{alertblock}
}
...
\begin{block}{课题小练习}
定义$\Z[\sqrt{2}] = \{a + b\sqrt{2}: a, b \in \Z\}$。证明：$\Z[\sqrt{2}]$在普通的加法和乘法下形成一个环。
\end{block}
...
\frame{\frametitle{Ring的属性.}
\begin{block}{课堂小练习}
证明：设$R$是带单位元的环，任取$a \in R$，则$(-1) a = -a$。
\end{block}

\pause

\begin{block}{课堂小练习}
证明：如果环$R$在加法群上是循环群，则$R$是交换环。
\end{block}
\iffalse
答案：因为$R$在加法群上是循环群，所以存在$r \in R$使得$R = \{kr: k \in \Z\}$。对任意的$a, b \in R$，存在整数$k_1, k_2$使得$a = k_1 r$和$b = k_2 r$，所以
\[ab = (k_1 r)(k_2 r) = k_1 k_2 r^2 = k_2 k_1 r^2 = (k_2 r)(k_1 r) = ba\]
因此，$R$是交换环。
\fi
\pause

\begin{block}{课堂小思考}
环中非零元素只需要满足结合律、封闭性。那么，乘法的什么律丢失了？如何补救呢？
\end{block}
}
...
即$\forall a, b \in R$，如果$ab = 0$则有$a = 0$或$b = 0$，
则称$R$为整环。
\end{definition}
}
...
\item 已知$\Z_n$是一个交换环。但是，$\Z_n$并不必然是整环。比如，$\Z_{15}$中，$(3*5) \bmod 15 = 0$，
...
\frame{\frametitle{整环属性}
\begin{block}{课堂小练习}
整环中有限阶幂等元素只有$0$和$1$。即证明：设$R$是整环，若$a^2 = a$，则$a=0$或$a=1$。
\end{block}
...
\begin{definition}
给定环$R$，$R'$是$R$的子集，如果$R'$在环$R$的加法和乘法上也形成环，则称$R'$是$R$的子环，记为$R' \subseteq R$。
\end{definition}
}
...
\begin{example}
\begin{enumerate}
\item 容易验证以下子环序列：$\Z\subseteq \mathbb{Q}\subseteq \mathbb{R}\subseteq \mathbb{C}$。

\item 偶数环是整数环的子环，即$2\Z \subseteq \Z$。由此可知，子环并不自然继承母环的单位元。当然，对任意的$n\in \Z$，有$n \Z \subseteq \Z$。
\end{enumerate}
\end{example}
}
...
\begin{definition}
给定两个环$R$和$R'$，若映射$\phi: R \to R'$满足：$\forall a, b \in R$，
\begin{align*}
\phi(a + b) &= \phi(a) + \phi(b)\\
\phi(ab) &= \phi(a)\phi(b)
\end{align*}
...
\begin{enumerate}
\item 已知$\Z$是环，定义映射$\phi : \Z \to \Z$为$\phi(k) = 2k$，$\forall k \in \Z$，即把所有整数映射到偶数$2\Z$。
已知$\phi$是$\Z$到$\Z$的群同态，但是可以验证$\phi$不是一种环同态。\pause

\item 对任意整数$n\in \Z$，已知$\Z_n$为环，定义映射$\phi : \Z_n \to \Z_n$为$\phi(a) = a^2 \bmod n$，$\forall a \in \Z_n$，
...
\item 考虑一种特殊的环同态，定义$\phi: 2\Z \to \Z_2$为$\forall k \in 2\Z$，$\phi(k) = k \bmod 2$。明显，$\phi$
是同态，但不是满同态，它把所有偶数都映射到了$0$。将这种把任意$R$映射到零环$\{0\}$的环同态称为\emph{零同态}。\pause

\item 考虑一种更特殊的环同态，对任意的环$R$，定义映射$\phi: R \to \Z_2$为$\forall a \in R$，$\phi(a) = 0$。可验证，
...
\item 对任意环$R$，已知$R\times R$是环。定义映射$\phi: R\times R \to R\times R$为
...
\item 设$p$是任意一个奇素数，考虑$2\Z$与$\Z_p$之间的映射$\phi: 2\Z \to \Z_p$，
...
环的理想在群论中的对应概念是正规子群。
\begin{definition}
给定环$R$，$I$是$R$的子环，如果对任意的$r \in R$有$rI \subseteq I$和$Ir \subseteq I$，则称$I$是$R$的理想。
...
\label{exm:ideal1}
\begin{enumerate}
\item 每个环$R$都有平凡理想$\{0\}$和$R$，其中在非零环中二者不同。
%R的平凡理想包括R本身，但是G的平凡子群通常不包括G。
...
\begin{theorem}
设$R$是环，$I$和$J$是$R$中的理想，并且$I + J = R$。 对任意$r_1, r_2 \in R$，以下方程组有解，并且，该方程组的任意两个解都模$I \cap J$同余。
\begin{align*}
x &\equiv r_1 \pmod{I}\\
x &\equiv r_2 \pmod{J}
\end{align*}
...
\frame{\frametitle{课堂训练：Ring and Ideal}
\begin{enumerate}
\item 判断$\Z_6$是否是带单位元的交换环，并找出其中的零因子。

\item 证明$2\Z$是$\Z$的子环。进一步判断：$2\Z$是否是$\Z$的理想？是否继承$\Z$的乘法单位元？

\item 在商环$\Z / 3\Z$中，计算$(2 + 3\Z) + (5 + 3\Z)$和$(2 + 3\Z)(5 + 3\Z)$，并写出结果对应的陪集。
\end{enumerate}
}

\frame{\frametitle{环同态与Kernel}
\begin{proposition}
环同态$\phi: R \to R'$的Kernel是$R$的理想。
\end{proposition}
...
\frame{\frametitle{环同态与Kernel}
\begin{example}
对任意整数$n\in \Z$，定义映射$\phi: \Z \to \Z_n$为，对任意$ a \in \Z$，$\phi(a) = a \bmod n$。容易验证
...
\begin{block}{第一}
环$R$本身在加法上是阿贝尔群，而其理想$I$则是$R$的正规加法子群，因此$R/I$在加法上就是一个商群，其中元素就是加法上$I$的陪集。比如，任取$r\in R$，$r + I$
就是$R/I$中的群元素。为清晰起见，定义$R/I$加法如下。对任意的$r, s \in R$，
\[(r + I) + (s + I) = (r+s) + I\]
\end{block}
...
\begin{lemma}
\label{lem:qr_mul}
设$R$是环，$I$是$R$中的理想。商群$R/I$中元素的乘法定义如下：
对任意$r, s\in R$，
\[(r + I)(s +I) = rs + I\]
...
\begin{definition}[标准同态]
	设$I$是环$R$的理想，定义环同态映射$\phi : R \to R / I$为：对任意$r \in R$，$\phi(r) = r + I$ 。
    并称该映射为环的\emph{标准同态}或者\emph{自然同态}，且$\ker \phi = I$。
\end{definition}

\begin{theorem}[第一同构定理]
设$\psi: R \to S$是环同态，记$K= \ker \psi$是$R$的理想。如果$\phi: R \to R/K$
是标准同态，则存在唯一同构$\eta: R/K \to \psi(R)$使得$\psi = \eta \phi$。
\end{theorem}
}
\frame{\frametitle{环的标准同态与第一同构定理}
\begin{proof}
根据群论的第一同构定理，在$R$的加法群与$R$模$K$的加法商群之间，
存在唯一的良定义的群同构$\eta: R/K \to \psi(R)$。
...
\begin{example}
\label{exm:Zn_mod_nZ}
任取$n \in \Z$，构造映射$\phi: \Z \to \Z_n$为，任取$a \in \Z$，$\phi(a) = a \bmod n$。可验证，这是
一个环同态映射，且是满射。$\ker\; \phi = n \Z$，因为所有$n$的倍数都映射为$0$。根据第一同构定理，
$\Z / n \Z \cong \Z_n$。
\end{example}
}
...
$\forall a \in R$且$a \neq 0$，则存在唯一的$a^{-1}\in R$使得$aa^{-1} = a^{-1}a =1$，则称这种代数结构
为域。
\end{definition}
}
...
\begin{example}
\begin{enumerate}
\item 在普通的加法与乘法上，$\mathbb{Q}, \mathbb{R}, \mathbb{C}$都是域。$\Z$不是域，因为非零整数在乘法上不成群。

\item 任意给定素数$p$，在模$p$的加法与乘法上$\Z_p$是域。因为$\Z_p$在加法上是阿贝尔群，而$\Z_p^*$在乘法上
也是阿贝尔群。对任意合数$n\in \Z$，$\Z_n$则不是域，因为$\Z_n \setminus \{ 0\}$乘法上不成群。
\end{enumerate}
\end{example}
}

\frame{\frametitle{域的乘法逆元与零因子}
\begin{proposition}[乘法逆元与零因子.]
对任意的域$\mathrm{F}$，任取$a, b \in F$，如果$ab = 0$则$a = 0$或者$b = 0$。即域中不存在零因子。
\end{proposition}
...
构造映射$\lambda_a : D^* \to D^*$为$\lambda_a(d) = ad$，$\forall d \in D^*$。
...
\frame{\frametitle{特征的属性}
\begin{block}{课堂小练习}
以上证明中，$(ab)1 = (a1)(b1)$这一步成立的原因是什么？
\end{block}

\pause
\begin{proof}
一种直观的思路。构造从$\Z$到整环$D$的自然同态$\phi: \Z \to D$为：$\phi(k) = k \cdot 1$。以上结论其实就是同态在乘法上操作保持：
\[(ab)1 = (a1) (b1)\]
...
\begin{proposition}[有限域的特征.]
阶为$n$的有限域$\mathrm{F}$的特征是一个素数$p$，且$p\mid n$。
\end{proposition}

\begin{proof}
因为有限域$\mathrm{F}$的阶为$n$，且$\mathrm{F}$是加法群，
所以对任意的$a\in F$，有$na = 0$。
所以，$\mathrm{F}$的特征必然是素数$p$，且$p \mid n$。
\end{proof}
}

\frame{\frametitle{有限域的阶}
以下不加证明给出另一个重要结论。
\begin{proposition}[有限域的阶]
如果有限域$\mathrm{F}$的特征是素数$p$，则$\mathrm{F}$的阶是$p^n$，$n$是某个正整数。
进一步，对任意的素数$p$和正整数$n$，存在阶为$p^n$的有限域，并且
所有的$p^n$阶有限域都同构。
\end{proposition}

\begin{example}
$p^n$阶的有限域也记为$\mathrm{GF}(p^n)$，$\mathrm{GF}$是Galois Field的缩写。
\end{example}
}

\subsection{域的理想}
\frame{\frametitle{域的理想}
\begin{proposition}[域的理想]
任何一个域$\mathrm{F}$的理想只有$0$和自己本身$\mathrm{F}$。
\end{proposition}
\begin{proof}
首先，已知$0$和$\mathrm{F}$都是$\mathrm{F}$的理想。
设$I$是域$\mathrm{F}$的非$0$理想，则存在非零元素$a \in I$。因为$\mathrm{F}$是域，则存在$a$的乘法逆元$a^{-1} \in F$。
根据理想的吸收性，$a^{-1}a = 1 \in I$。包含$1$的理想$I$等于$\mathrm{F}$，即$I = F$。
\end{proof}
}

\frame{\frametitle{域同态是单射}
\begin{proposition}[域同态是单射.]
任何一个域同态或者是单射或者是零同态。
\end{proposition}

\begin{proof}
域$\mathrm{F}_1$到域$\mathrm{F}_2$的域同态$\phi$是单射，当且仅当$\ker\; \phi = \{0\} \subseteq \mathrm{F}_1$。
因为$\mathrm{F}_1$的理想只有$0$和$\mathrm{F}_1$本身，所以当$\ker \phi = \{0\} $时$\phi$是单射，
而当$\ker \phi = \mathrm{F}_1$时，$\phi$是零同态。
\end{proof}
}
...
即如果$M$是$R$的极大理想，则对$R$的任意理想$I$，若$M \subseteq I$，则$I = R$。
\end{definition}
\pause

\begin{definition}[素理想.]
    设$P$是交换环$R$的真理想，如果对任意$a,b\in R$，若$ab \in P$，则或者$a \in P$，或者$b \in P$，
...
\frame{\frametitle{极大理想、素理想、域与整环}
\begin{center}
...
\[
M\text{ 极大}
\Leftrightarrow R/M\text{ 是域}
\Rightarrow R/M\text{ 是整环}
\Leftrightarrow M\text{ 素}.
\]

\begin{block}{一条线索}
理想越“大”，商环越“小”也越“好”：极大理想对应域，素理想对应整环；而域一定是整环。
\end{block}

}

\frame{\frametitle{课程结束}
本学期课程到此结束。请尽早开始复习，梳理定义、定理和证明。
}
\end{document}
