...
\[\operatorname{Ker}\; \phi = \{r \in R: \phi(r) = 0\} \]
...
\begin{proof}
根据群论的结论，$K = \operatorname{Ker}\;\phi$是$R$的加法子群（并且是正规子群）。
...
这是一个环同态，而$\operatorname{Ker}\;\phi$就是$n\Z$。
\end{example}
%20200710
...
\begin{definition}{环的标准同态}{ring_can_homo}
    ...
    并称该映射为环的\emph{标准同态}或者\emph{自然同态}，且$\operatorname{Ker}\; \phi = I$。 
\end{definition}

\begin{theorem}{第一同构定理}{ring_first_iso}
设$\psi\colon R \to S$是环同态，记$K= \operatorname{Ker} \psi$是$R$的理想。如果$\phi\colon R \to R/K$
...
一个环同态映射。$\operatorname{Ker}\; \phi = n \Z$，因为所有$n$的倍数都映射为$0$。根据第一同构定理，
$\Z / n \Z \cong \Z_n$。
\end{example}
...
比如，考虑一个从域$\F_1$到域$\F_2$的域单射同态$\phi$，$\operatorname{Ker}\; \phi$会是什么样？
单射当然意味着$\operatorname{Ker}\; \phi$只有一个元素$0$。$\operatorname{Ker}\; \phi$是什么？域的理想！
...
\begin{proof}
域$\F_1$到域$\F_2$的域同态$\phi$是单射，当且仅当$\operatorname{Ker}\; \phi = \{0\} \subset \F_1$。
因为$\F_1$的理想只有$0$和$\F_1$本身，所以当$\operatorname{Ker}\; \phi = \{0\} $时$\phi$是单射，
而当$\operatorname{Ker}\; \phi = \F_1$时，$\phi$是零同态。
\end{proof}
