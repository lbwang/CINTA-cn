...
\title{ Mathematical Foundations in Information Security -- 2} 
\author{Libin Wang} 
...
\begin{frame}{同余的直观}
	\centering
	\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{../figures/chapt3-1-mod-congruence-buckets.png}
\end{frame}
...
\frame{\frametitle{More about Euler's Phi Function.}
\begin{remark}[Why the conclusion holds?]
   If $\gcd(a, m)=1$ and $\gcd(a, n)=1$ then $\gcd(a, mn) = 1$.
\end{remark}
}

\frame{\frametitle{More about Euler's Phi Function.}
\begin{remark}[Why the second item holds?]
